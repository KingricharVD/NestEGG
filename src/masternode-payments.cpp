...
{
    CMasternode* pmn = CConnman.Find(vinMasternode);
    ...
    if (!pmn) {
        ...
        CConnman.AskForMN(pnode, vinMasternode);
        return false;
    }
    ...
    int n = CConnman.GetMasternodeRank(vinMasternode, nBlockHeight - 100, ActiveProtocol());
    ...
    if (n > signaturesTotal) {
        ...
        if (n > signaturesTotal * 2) {
            strError = strprintf("Masternode not in the top %d (%d)", signaturesTotal * 2, n);
            LogPrint(BCLog::MASTERNODE, "CMasternodePaymentWinner::IsValid - %s\n", strError);
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, const CBlockIndex* pindexPrev, bool fProofOfStake)
{
    ...
    if (!masternodePayments.GetBlockPayee(pindexPrev->nHeight + 1, payee)) {
        ...
        CMasternode* winningNode = CConnman.GetCurrentMasterNode(1);
        if (winningNode) {
...
void CMasternodePayments::ProcessMessageMasternodePayments(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == NetMsgType::GETMNWINNERS) { //Masternode Payments Request Sync
        ...
        int nFirstBlock = nHeight - (CConnman.CountEnabled() * 1.25);
...
if (Params().GetConsensus().NetworkUpgradeActive(chainActive.Tip()->nHeight, Consensus::UPGRADE_TIME_PROTOCOL_V2) &&
        ...
        if (!winner.CheckSignature()) {
            ...
            CConnman.AskForMN(pfrom, winner.vinMasternode);
            return;
        }
...
void CMasternodePayments::CleanPaymentList()
{
    ...
    int nLimit = std::max(int(CConnman.size() * 1.25), 1000);
...
bool CMasternodePayments::ProcessBlock(int nBlockHeight)
{
    ...
    int n = CConnman.GetMasternodeRank(*(activeMasternode.vin), nBlockHeight - 100, ActiveProtocol());
    ...
    if (budget.IsBudgetPaymentBlock(nBlockHeight)) {
        ...
        int nCount = 0;
        CMasternode* pmn = CConnman.GetNextMasternodeInQueueForPayment(nBlockHeight, true, nCount);
...
void CMasternodePayments::Sync(CNode* node, int nCountNeeded)
{
    ...
    int nCount = (CConnman.CountEnabled() * 1.25);
