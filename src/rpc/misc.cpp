...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"isstaking\" : true|false,       (boolean) If the address is a staking address for SAPP cold staking -only if is standard address-\n"
...
UniValue getstakingstatus(const JSONRPCRequest& request)
{
        ...
        throw JSONRPCError(RPC_IN_WARMUP, "Try again after active chain is loaded");
    {
        ...
        obj.push_back(Pair("stakingbalance", ValueFromAmount(pwalletMain->GetColdStakingBalance(fColdStaking))));
