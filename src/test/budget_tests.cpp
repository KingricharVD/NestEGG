...
BOOST_AUTO_TEST_CASE(budget_value)
{
    ...
    int nHeightTest = Params().GetConsensus().vUpgrades[Consensus::UPGRADE_V4_0].nActivationHeight + 1;
    CheckBudgetValue(nHeightTest, "testnet", 7300*COIN);

    SelectParams(CBaseChainParams::MAIN);
    nHeightTest = Params().GetConsensus().vUpgrades[Consensus::UPGRADE_V4_0].nActivationHeight + 1;
