...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected HCCP and zHCCP supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zHCCPsupply']:
                assert_equal(node['zHCCPsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check HCCP and zHCCP supply at the beginning
        # ------------------------------------------
        # zHCCP supply: 2 coins for each denomination
        ...
        # HCCP supply: block rewards minus burned fees for minting
        ...
        # Verify that HCCP and zHCCP supplies were properly updated after the spends and reorgs
        self.log.info("Check HCCP and zHCCP supply...")
